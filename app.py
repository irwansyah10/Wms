...
DB_URL = "sqlite:///wms_streamlit.db"  # storage lokal/ephemeral di Streamlit Cloud
...
# ---------------- Utils ----------------
def do_rerun():
    # Kompatibel untuk Streamlit versi baru & lama
    if hasattr(st, "rerun"):
        st.rerun()
    else:
...
class User(Base):
    ...
    role = Column(String(32), nullable=False, default="picker")  # admin, supervisor, picker
...
# ---------------- DB init & seed ----------------
def seed(session):
    # Seed inventory/receipts/orders
    if not session.scalar(select(func.count(Item.id))):
        items = [
            Item(sku="SKU-001", name="Cardboard Box Small", barcode="1000001",
                 bin_location="A1-01", reorder_point=20, on_hand=100),
            Item(sku="SKU-002", name="Bubble Wrap 50m", barcode="1000002",
                 bin_location="A1-02", reorder_point=10, on_hand=25),
            Item(sku="SKU-003", name="Packing Tape", barcode="1000003",
                 bin_location="B1-01", reorder_point=15, on_hand=60),
        ]
...
def topbar():
    ...
    with col1:
        st.markdown(f"### 📦 WMS — **{u}**  <span style='font-size:0.9em'>(**{role}**)</span>", unsafe_allow_html=True)
    with col2:
        if st.button("Logout"):
...
def page_dashboard():
    ...
    with SessionLocal() as s:
        ...
        status_counts = {k: s.scalar(select(func.count(Order.id)).where(Order.status == k))
                         for k in ["open","picking","packed","shipped","closed"]}
        shipped = s.execute(select(Order).where(Order.shipped_at.is_not(None))).scalars().all()
        avg_hours = round(sum([(o.shipped_at - o.created_at).total_seconds()/3600.0
                               for o in shipped])/len(shipped), 2) if shipped else None
    ...
    if low_stock:
        df = pd.DataFrame([{
            "SKU": i.sku, "Name": i.name, "OnHand": i.on_hand,
            "ROP": i.reorder_point, "Bin": i.bin_location
        } for i in low_stock])
...
def page_inventory():
    ...
    df = pd.DataFrame([{
        "SKU": i.sku, "Name": i.name, "Barcode": i.barcode,
        "Bin": i.bin_location, "ROP": i.reorder_point, "On Hand": i.on_hand
    } for i in items])
    st.dataframe(df, use_container_width=True)
            ...
            if not sku or not name:
                ...
                with SessionLocal() as s:
                    if s.execute(select(Item).where(Item.sku == sku)).scalar_one_or_none():
                        ...
                        s.add(Item(sku=sku, name=name, barcode=barcode or None,
                                   bin_location=bin_loc or None, reorder_point=int(rop), on_hand=0))
                        s.commit()
...
def page_inbound():
    ...
    # create receipt (admin/supervisor)
    ...
    if st.session_state.get("role") in ("admin","supervisor"):
        ...
        st.info("Only admin/supervisor can create receipts.")
    ...
    for r in receipts:
        with st.expander(f"{r.ref} — {r.vendor or '—'}  [{r.status}]"):
            ...
            if st.session_state.get("role") in ("admin","supervisor"):
                with st.form(f"add_line_{r.id}"):
                    ...
                    if st.form_submit_button("Add Line"):
                        with SessionLocal() as s:
                            s.add(ReceiptLine(receipt_id=r.id, item_id=opts[label], qty=int(qty)))
                            s.commit()
            ...
            if lines:
                df = pd.DataFrame([{
                    "SKU": ln.item.sku, "Name": ln.item.name,
                    "Qty": ln.qty, "Received": ln.received_qty, "LineID": ln.id
                } for ln in lines])
                ...
                for ln in lines:
                    if st.session_state.get("role") in ("admin","supervisor"):
                        with st.form(f"recv_{ln.id}"):
                            recv = st.number_input(f"Receive qty for {ln.item.sku}", min_value=1, step=1, value=1)
            ...
            if st.session_state.get("role") in ("admin","supervisor"):
                if st.button("Close Receipt", key=f"close_{r.id}"):
                    with SessionLocal() as s:
                        rr = s.get(Receipt, r.id)
                        rr.status = "received"
                        s.commit()
                        st.success("Receipt closed.")
                        do_rerun()
...
def page_orders():
                ...
                if not ref:
                    ...
                    with SessionLocal() as s:
                        if s.execute(select(Order).where(Order.ref == ref)).scalar_one_or_none():
                            ...
                            s.add(Order(ref=ref, customer=customer or None))
                            s.commit()
    ...
    for o in orders:
        with st.expander(f"{o.ref} — {o.customer or '—'}  [{o.status}]"):
            ...
            if lines:
                df = pd.DataFrame([{
                    "SKU": l.item.sku, "Name": l.item.name, "Ordered": l.qty,
                    "Picked": l.picked_qty, "Bin": l.item.bin_location, "LineID": l.id
                } for l in lines])
                ...
                for l in lines:
                    if st.session_state.get("role") in ("admin","supervisor","picker"):
                        with st.form(f"pick_{l.id}"):
                            pick = st.number_input(f"Pick qty for {l.item.sku}", min_value=1, step=1, value=1)
                            if st.form_submit_button("Pick"):
                                with SessionLocal() as s:
                                    ...
                                    if pick > item.on_hand:
                                        ...
                                        oo = s.get(Order, o.id)
                                        oo.status = "picking"
                                        oo.picked_at = datetime.utcnow()
                                        s.commit()
                                        st.success("Picked recorded.")
                                        do_rerun()
...
def page_users_admin():
    ...
    with SessionLocal() as s:
        users = s.execute(select(User).order_by(User.username)).scalars().all()
    if users:
        label_map = {f"{u.username} ({u.role})": u.id for u in users}
        sel = st.selectbox("Select User", list(label_map.keys()))
        uid = label_map[sel]
        with SessionLocal() as s:
            u = s.get(User, uid)

        new_username = st.text_input("New Username", value=u.username, key=f"nu_{uid}")
        new_role = st.selectbox("New Role", ["admin","supervisor","picker"],
                                index=["admin","supervisor","picker"].index(u.role),
                                key=f"nr_{uid}")
        col1,col2,col3 = st.columns(3)
        with col1:
            if st.button("Save"):
...
def main():
    ...
    if st.session_state.get("role") == "admin":
        pages["Users (Admin)"] = page_users_admin
